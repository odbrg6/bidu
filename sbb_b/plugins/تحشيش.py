...
async def permalink(mention):
    ...
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"**᯽︙ المستخدم** [{jmthon}](tg://user?id={user.id}) \n**᯽︙  تـم رفعـها حاته القروب 🤤😻 بواسطة :** {my_mention} \n**᯽︙  تعاي يعافيتي اريد حضن دافي 😽**")
...
async def permalink(mention):
    ...
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"**᯽︙ المستخدم** [{jmthon}](tg://user?id={user.id}) \n**᯽︙  تـم رفعـه زبال القروب 🧹 بواسطة :** {my_mention} \n**᯽︙  تعال يلزبال اكنس القروب لا أهينك 🗑😹**")
...
async def permalink(mention):
    ...
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"**᯽︙ المستخدم** [{jmthon}](tg://user?id={user.id}) \n**᯽︙  تـم رفعه ديوث القروب بواسطة :** {my_mention} \n**᯽︙  تعال يلديوث جيب اختك خلي اتمتع وياها 🔞**")
...
async def permalink(mention):
    ...
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"**᯽︙ الحلو** 「[{jmthon}](tg://user?id={user.id})」 \n**᯽︙  تـم رفعه مالك القروب بواسطة :** {my_mention}")
...
async def permalink(mention):
    ...
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"** ᯽︙  المستخدم => • ** [{jmthon}](tg://user?id={user.id}) \n ☑️ **᯽︙  تم رفعه وصخ القروب 🤢 بواسطه  :**{my_mention} .\n**᯽︙  لك دكوم يلوصخ اسبح مو ريحتك كتلتنا 🤮 ** ")
